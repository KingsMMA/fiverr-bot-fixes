...
const command: SlashCommand = {
    command: new SlashCommandBuilder()
        .setName("remove")
        .setDescription("remove dobby points from a designated user | mods-only")
    ...
    execute: async (interaction) => {
        try {
            ...
            await prisma.player.upsert({
                ...
                update: {
                    discordTag,
                    dobbyPoints: { decrement: amount }
                }
            });

            const content = `${userMention(interaction.user.id)} removed ${amount} ${POINTS_EMOJI} from ${userMention(
                discordID
            )}!`;
            await interaction.editReply({ content });
            await log({
                title: "Dobby Points Removed",
                content,
                color: "Red"
            });
        } catch (err: unknown) {
